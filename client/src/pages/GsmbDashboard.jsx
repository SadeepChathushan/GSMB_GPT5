...
import { Users, Truck, Shield, Plus, User, Calendar, Phone, Mail, Building2, AlertTriangle, CheckCircle2, Pause, Play } from "lucide-react";
...
export default function GsmbDashboard() {
  ...
  async function createOwner() {
    try {
      await mockApi.post("/gsmb/owners", { email: oEmail, name: oName, phone: oPhone, password: oPass });
      ...
      setOPhone("");
      setOPass("Temp@12345");
      showNotification("Owner registered successfully!", "success");
    } catch (error) {
      showNotification("Failed to register owner", "error");
    }
  }

  async function createLicense() {
    try {
      await mockApi.post("/gsmb/licenses/new", { ownerEmail, lorryNumber: lorry, siteName: site, expiresAt });
      ...
      showNotification("Failed to create license", "error");
    }
  }
  ...
  async function suspend(licenseId) {
    try {
      ...
      const { data } = await mockApi.get("/gsmb/licenses");
      setLicenses(data);
      showNotification("License suspended", "warning");
    } catch (error) {
      showNotification("Failed to suspend license", "error");
    }
  }
  ...
  async function reinstate(licenseId) {
    try {
      ...
      const { data } = await mockApi.get("/gsmb/licenses");
      setLicenses(data);
      showNotification("License reinstated", "success");
    } catch (error) {
      showNotification("Failed to reinstate license", "error");
    }
  }
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case "SUSPENDED": return "from-orange-500 to-red-500";
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-6">
              {["overview", "licenses", "owners"].map((tab) => (
                <button
                  key={tab}
        ...
        {(activeTab === "overview" || activeTab === "owners") && (
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
            ...
            <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 border border-blue-200 shadow-2xl">
              ...
              <div className="space-y-4">
                <div className="relative group">
                  <Mail className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-blue-400 group-focus-within:text-blue-600 transition-colors" />
                  <input 
                ...
                <div className="relative group">
                  <User className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-blue-400 group-focus-within:text-blue-600 transition-colors" />
                  <input 
                    className="w-full bg-blue-50/50 border border-blue-200 rounded-xl pl-12 pr-4 py-4 text-slate-800 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition-all duration-300"
                ...
                <div className="relative group">
                  <Phone className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-blue-400 group-focus-within:text-blue-600 transition-colors" />
                  <input 
                    className="w-full bg-blue-50/50 border border-blue-200 rounded-xl pl-12 pr-4 py-4 text-slate-800 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition-all duration-300"
                ...
                <div className="relative group">
                  <Shield className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-blue-400 group-focus-within:text-blue-600 transition-colors" />
                  <input 
                    type="password"
                    className="w-full bg-blue-50/50 border border-blue-200 rounded-xl pl-12 pr-4 py-4 text-slate-800 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition-all duration-300"
              ...
              <div className="mt-8">
                ...
                <div className="space-y-3 max-h-48 overflow-auto">
                  {owners.map(o => (
                    <div key={o._id} className="bg-blue-50/50 rounded-xl p-4 border border-blue-100 hover:bg-blue-100/50 transition-all duration-300">
                      <div className="flex items-center justify-between">
                        <div>
            ...
            <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 border border-blue-200 shadow-2xl">
              ...
              <div className="space-y-4">
                <div className="relative group">
                  <Mail className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-indigo-400 group-focus-within:text-indigo-600 transition-colors" />
                  <input 
                    className="w-full bg-indigo-50/50 border border-indigo-200 rounded-xl pl-12 pr-4 py-4 text-slate-800 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 transition-all duration-300"
                    placeholder="Owner Email" 
                    value={ownerEmail} 
                ...
                <div className="relative group">
                  <Truck className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-indigo-400 group-focus-within:text-indigo-600 transition-colors" />
                  <input 
                    className="w-full bg-indigo-50/50 border border-indigo-200 rounded-xl pl-12 pr-4 py-4 text-slate-800 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 transition-all duration-300"
                ...
                <div className="relative group">
                  <Building2 className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-indigo-400 group-focus-within:text-indigo-600 transition-colors" />
                  <input 
                    className="w-full bg-indigo-50/50 border border-indigo-200 rounded-xl pl-12 pr-4 py-4 text-slate-800 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 transition-all duration-300"
                ...
                <div className="relative group">
                  <Calendar className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-indigo-400 group-focus-within:text-indigo-600 transition-colors" />
                  <input 
                    className="w-full bg-indigo-50/50 border border-indigo-200 rounded-xl pl-12 pr-4 py-4 text-slate-800 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 transition-all duration-300"
                    type="date" 
                    value={expiresAt} 
                ...
                <button 
                  className="w-full bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white font-semibold py-4 px-6 rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-indigo-500/25 flex items-center justify-center space-x-2"
        ...
        {(activeTab === "overview" || activeTab === "licenses") && (
          <div className="space-y-6">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-3xl font-bold text-slate-800">Mining Licenses</h2>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
              {licenses.map(l => (
                <div key={l._id} className="group bg-white/80 backdrop-blur-xl rounded-2xl p-6 border border-blue-200 shadow-xl hover:bg-white/90 transition-all duration-500 transform hover:scale-105 hover:shadow-2xl">
                  {/* Status Badge */}
                  <div className={`inline-flex items-center space-x-2 px-3 py-2 rounded-full bg-gradient-to-r ${getStatusColor(l.status)} text-white text-xs font-semibold mb-4`}>
                  ...
                  <div className="space-y-3 mb-6">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2 text-slate-600">
                        ...
                        <span className="text-sm">{l.owner?.name}</span>
                      </div>
                      <div className="flex items-center space-x-2 text-slate-600">
                        <Mail className="w-4 h-4" />
                        <span className="text-sm">{l.owner?.email}</span>
                      </div>
                      <div className="flex items-center space-x-2 text-slate-600">
                        <Building2 className="w-4 h-4" />
                        <span className="text-sm">{l.siteName}</span>
                      </div>
                      <div className="flex items-center space-x-2 text-slate-600">
                        <Calendar className="w-4 h-4" />
                        <span className="text-sm">Expires: {l.expiresAt}</span>
                      </div>
                    </div>
                  </div>
        ...
        {activeTab === "owners" && (
          <div className="space-y-6">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-3xl font-bold text-slate-800">All Owners</h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {owners.map(owner => (
                <div key={owner._id} className="bg-white/80 backdrop-blur-xl rounded-2xl p-6 border border-blue-200 shadow-xl hover:bg-white/90 transition-all duration-300 transform hover:scale-105">
