...
const MockNavbar = () => (
  <div className="p-4 font-semibold text-center bg-white shadow-sm">MMPRO</div>
);

export default function Login() {
  // Mock hooks for demo - use your actual hooks
  const mockLogin = async (email, password) => ({ role: "GSMB" });
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (user.role === "POLICE") mockNavigate("/police");
      else if (user.role === "GSMB") mockNavigate("/gsmb");
      else if (user.role === "OWNER") mockNavigate("/owner");
      else mockNavigate("/");
    } finally {
      setLoading(false);
    }
    ...
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100">
      ...
      <div className="flex items-center justify-center flex-1 px-4">
        <div className="w-full max-w-md overflow-hidden bg-white border border-gray-100 shadow-xl rounded-2xl">
          ...
          {/* Body
              - On small screens: allow internal scroll (so page can stay comfortable)
              - On md+ screens: no internal scroll, fits without page scroll */}
